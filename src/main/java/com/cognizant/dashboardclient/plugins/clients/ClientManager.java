...
public class ClientManager {
    ...
    public static Map<String, String> addImage(File file, String title) throws IOException {
        if (title == null || "".equals(title)) title = file.getName();
        DiskFileItem fileItem = new DiskFileItem
                ("file", "image/png", false, file.getName(),(int) file.length(), file.getParentFile());
        ...
        Map<String, String> id = getUploadClient().addImage(title, multipartFile, uploadHeaders);
        return id;
    }
